...
@section('title', "Adopter un chaton Bengal — parcours et pré-réservation")
@section('description', "Le parcours d'adoption chez Bengal's Parc en quatre étapes, le détail de ce que couvre l'adoption, et le formulaire de pré-réservation.")
...
<section class="band">
    <div class="wrap">
        ...
        <div class="cells">
            ...
            <div class="cell-b"><span class="n">ÉTAPE 02</span><h3>Vous venez les voir</h3><p>Visite sur rendez-vous à L'Isle d'Abeau. Vous rencontrez la mère, la fratrie complète, et vous voyez l'endroit où ils grandissent.</p></div>
...
<section class="band paper" id="couverture">
    <div class="wrap">
        ...
        <div class="ledger">
            @foreach(config('bengal.couverture') as $ligne)
                <div class="row">
            ...
            <form class="demo" method="POST" action="{{ route('adoption.store') }}">
                ...
                <div class="field"><label for="f-exp">Expérience avec les chats</label>
                    <select id="f-exp" name="experience">
                        @foreach(['Premier chat', 'J\'ai déjà eu des chats', 'J\'ai déjà eu un Bengal'] as $choix)
                ...
                <div class="field full"><label for="f-msg">Votre message</label>
                    <textarea id="f-msg" name="message" placeholder="Parlez-nous de votre rythme de vie, de ce que vous attendez d'un Bengal, de vos questions…">{{ old('message') }}</textarea></div>

                <label class="consent" for="f-rgpd">
                    <input type="checkbox" id="f-rgpd" name="rgpd" value="1" @checked(old('rgpd'))>
                    <span>
                        J'accepte que Bengal's Parc conserve ces informations pour traiter ma demande
